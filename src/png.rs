...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
            ...
            .position(|chunk| chunk.chunk_type().to_string() == chunk_type)
        {
            ...
            Err("No chunk of that type found".into())
        }
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
        if value[0..8] != Png::STANDARD_HEADER {
            return Err("Standard PNG header not detected.".into());
        }
        let mut walking_index = Png::STANDARD_HEADER.len();
        let mut chunks = vec![];
        while let Some(bytes) = value.get(walking_index..) {
            if bytes.is_empty() {
                break;
            }
